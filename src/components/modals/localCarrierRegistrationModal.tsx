...
// import { registerPickupCarrier } from "../../redux/actions/pickup-carrier";
import { unwrapResult } from "@reduxjs/toolkit";
import VerifyOtpModal from "./verifyOtpModal";
import AddZipCodeModal from "./addZipCodeModal";
import {
  checkIfLocalCarrierExists,
  registerLocalCarrier,
} from "../../redux/actions/local-carrier";
...
const LocalCarrierRegistrationModal = ({
  localCarrierRegistrationModalVisible,
  setLocalCarrierRegistrationModalVisible,
  setIsVerifyOtpModal,
  isVerifyOtpModal,
  setSuccess,
  success,
  isLoading,
  setIsLoading,
  handleCompleteRegistrationLocalCarrier,
  ...
  const handleFileChange = (event: any, setFile: any) => {
    ...
    if (file && file.type === "application/pdf") {
      // Handle the file (set it to state, etc.)
      setFile(file);
    } else {
  ...
  const handleInputOperationalState = (state: any) => {
    if (operatingStates.includes(state)) {
      ...
      setStatePayload((prevData: any) => [...prevData, stateInfo]);
      setOperatingStates([...operatingStates, state]);
    }
  };
  const [statePayload, setStatePayload] = useState<any>([]);
  const [infoForState, setInfoForState] = useState("");
          ...
          if (companyState === null) {
            ...
            if (companyCity === null) {
              ...
              if (companyZipCode.length !== 5) {
                setFieldError(
                  "Please enter at least 5-digits for Company Zip Code."
                );
              } else {
  ...
  const handleStepTwo = () => {
    if (movingCompanyOfficialLicense === null) {
      setFieldError("Please upload Company Tariff document in PDF format.");
    } else {
      if (movingCompanyOfficialLicense.size > 5000000) {
        ...
        if (movingCompanyOfficialLicense.type !== "application/pdf") {
          setFieldError(
            "Please select a PDF file for Company Tariff document."
          );
        } else {
  ...
  const handleStepThree = () => {
    if (statePayload.length === 0) {
      ...
      if (trucksInOperation === null || trucksInOperation < 0) {
        setFieldError(
          "Please enter valid number of trucks you have in operation."
        );
      } else {
        if (yearsInBusiness === null || yearsInBusiness < 0) {
          ...
          if (insuranceCompanyName === "") {
            ...
            if (insuranceCompanyPhoneNumber.length !== 10) {
              setFieldError(
                "Please enter at least 10-digit Insurance Company Phone Number."
              );
            } else {
  ...
  const checkIfAccountExists = () => {
    ...
    dispatch<any>(checkIfLocalCarrierExists(body))
      .then(unwrapResult)
      .then((res: any) => {
        setIsLoading(false);
        console.log(res);
        if (res === true) {
          if (step === 1) {
            setFieldError(
              "Account with this Company Phone Number already exists."
            );
          }
          if (step === 2) {
            setFieldError(
  ...
  const createLocalRegisterCarrier = () => {
    setWaitingMessage("Onboarding you as a Local Carrier. Please wait...");
    const body = {
    ...
    handleCompleteRegistrationLocalCarrier(body);
  };

  const [addZipCodeModal, setAddZipCodeModal] = useState(false);
  ...
  return (
    <>
      <Modal
        open={localCarrierRegistrationModalVisible}
        onClose={handleClose}
        ...
        >
          ...
          {infoLostAlertModalVisible && (
            <InfoLostAlertModal
              ...
              setAnyModal={setLocalCarrierRegistrationModalVisible}
            />
          )}
          {isLoading && (
            <Backdrop
          ...
          {addZipCodeModal && (
            <AddZipCodeModal
              ...
              infoForState={infoForState}
              statePayload={statePayload}
              setStatePayload={setStatePayload}
              addZipCodeModal={addZipCodeModal}
              setAddZipCodeModal={setAddZipCodeModal}
            />
          )}
          {loading && (
          ...
          {isVerifyOtpModal && (
            <VerifyOtpModal
              ...
              setParentModal={setLocalCarrierRegistrationModalVisible}
            />
          )}
          {success && (
              ...
              <Box component="form" p={3}>
                      ...
                      operatingStates.map((thisState: any) => {
                        return (
                          ...
                          >
                            ...
                            {statePayload.map((stateInfo: any) => {
                              if (
                                thisState.name ===
                                stateInfo.pickup_service_state
                              ) {
                                return (
                                  <Stack
                                    spacing={1}
                                    direction={"row"}
                                    key={stateInfo.pickup_service_state}
                                  >
                                    {stateInfo.zip_codes.map((zipCode: any) => (
                                      <Typography
                                        fontWeight={600}
                                        fontSize={12}
                            ...
                            {/* <Button
                              sx={{ width: 90 }}
                              onClick={() => {
                                setInfoForState(thisState.name);
                                setAddZipCodeModal(true);
                              }}
                            >
                  ...
                  <Stack>
                    ...
                    <Stack mt={1} direction="row" spacing={1}>
                      ...
                      <Button
                        ...
                        sx={
                          haveTrailers ? selectedButtonStyle : notSeelectedButtonStyle
                        }
                      >
                        Yes
                      </Button>
                ...
                >
                  <TextField
                    ...
                    size="small"
                    required
                    value={insuranceCompanyName ? insuranceCompanyName : ""}
                    onChange={(e) => {
                  ...
                  <TextField
                    label="Phone of Insruance Company"
                    variant="outlined"
                    onChange={(event) => {
...
export default LocalCarrierRegistrationModal;
